...
import {Dashboard,People,DirectionsCar,Healing, Build,DesignServices, Assignment,CalendarMonth,ExpandLess,ExpandMore,AttachMoney,Construction,Group,Settings,HelpOutline,CreditCard,ListAlt,Security,Email,Person,Palette,Language,Notifications,Business,
  Description,AccessTime,TrendingUp,Assessment,BarChart,ShowChart,Inventory,Chat,Receipt,LocalOffer,LibraryBooks,TrackChanges,LocalShipping,Badge,BeachAccess,Work,CarRepair,ManageAccounts,
} from "@mui/icons-material"
import { supabase } from "@/lib/supabase";

const fetchAvatarUrl = async (fileName) => {
  if (!fileName) {
    throw new Error("Nom de fichier manquant");
  }
  const { data, error } = supabase.storage.from('avatar').getPublicUrl(fileName);
...
export const useAvatarUrl = (fileName) => {
  return useQuery({
    queryKey: ['avatar', fileName],
    queryFn: () => fetchAvatarUrl(fileName), 
    enabled: !!fileName, 
    staleTime: Infinity, 
    retry: 1, 
  });
...
const Sidebar = () => {
  ...
  <div className="footer">
    <img src={user?.avatar_url || "/placeholder.svg"} alt="User Avatar" className="footer-avatar" />
    <div className="footer-user-info">
      <div className="footer-user-name">{user?.full_name || "Utilisateur"}</div>
      <div className="footer-user-role">{user?.role || "Rôle"}</div>
    </div>
  </div>
</div>
